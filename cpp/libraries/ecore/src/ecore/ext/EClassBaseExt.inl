...
namespace ecore::ext
{
                ...
                if( featureID == EcorePackage::ECLASS__ESUPER_TYPES )
                {
                    ...
                    case ENotification::RESOLVE:
                    {
                        ...
                        if( !oldValue.empty() )
                        {
                            ...
                            auto it = std::find_if(
                                subClasses.begin(), subClasses.end(), [=]( const auto& w ) { return w.lock() = eNotifier; } );
    ...
    template <typename... I>
    template <typename U>
    class EClassBaseExt<I...>::EObjectInternalAdapter : public ecore::impl::EClassBase<I...>::EObjectInternalAdapter<typename U>
    {
    public:
        EObjectInternalAdapter( EClassBaseExt<I...>& obj )
            : ecore::impl::EClassBase<I...>::EObjectInternalAdapter<typename U>( obj )
        {
        }

        virtual void setModified( int featureID )
        {
            getObject().setModified( featureID );
        }
    ...
    template <typename... I>
    std::unique_ptr<impl::EObjectInternal> EClassBaseExt<I...>::createInternal()
    {
        return std::move( std::make_unique<EObjectInternalAdapter<EClassInternal>>( *this ) );
    }
