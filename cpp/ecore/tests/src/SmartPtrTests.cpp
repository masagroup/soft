...
BOOST_AUTO_TEST_CASE( DerivedPointerCast_Static )
{
    ...
    auto b1 = derived_pointer_cast<B, B>( b );
    BOOST_CHECK_EQUAL( b, b1 );

    auto b2 = derived_pointer_cast<A, B>( a );
    BOOST_CHECK_EQUAL( b, b2 );

    auto a2 = derived_pointer_cast<B, A>( b );
...
BOOST_AUTO_TEST_CASE( DerivedPointerCast_Dynamic )
{
    ...
    auto c1 = derived_pointer_cast<C, C>( c );
    BOOST_CHECK_EQUAL( c, c1 );

    auto c2 = derived_pointer_cast<A, C>( a );
    BOOST_CHECK_EQUAL( c, c2 );

    auto a2 = derived_pointer_cast<C, A>( c );
    BOOST_CHECK_EQUAL( a, a2 );
}
